...
def clear_tasks():
    app.logger.info("start clear tasks---------------")
    project = request.args.get('project', "")
    app.logger.info("clear tasks---------------" + project)
    projectdb = app.config['projectdb']
    project_info = projectdb.get(project, fields=['name', 'status'])
    if project_info.get('status') != "STOP":
        app.logger.info("clear tasks failed, project not stop----------------")
        return 'project not stop', 200
    taskdb = app.config['taskdb']
    taskdb.truncate(project)
    return '', 200
